<template>
    <a :preserve-scroll="true" @click.prevent="registerUser()" :disabled="!votingPowerCheck && votingPowerCheck !== null"
       class="absolute top-0 left-0 flex items-center justify-center w-full h-full p-8 text-sky-700 rounded-lg bg-sky-800/50 hover:cursor-pointer">
        <div class="flex flex-col transition ease-in-out delay-150 px-4 py-2 text-lg font-bold bg-sky-100 rounded-full xl:text-text hover:-translate-y-1 hover:scale-110 duration-300" :class="{'opacity-90 cursor-not-allowed': !votingPowerCheck && votingPowerCheck !== null}">
...
watch(wallet, () => {
    if (wallet.value?.stakeAddress && props.ballot?.hash) {
        voterStore
            ...
            .then(() => {
                console.log(voterPowers.value[props.ballot.hash]);
