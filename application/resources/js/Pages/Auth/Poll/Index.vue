            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="sm:rounded-lg">
                    ...
                    <div class="w-full">
                        ...
                        <div v-if="pollsPagination && pollsData.length > 0"
                            class="flex flex-row items-center justify-between w-full py-4 mt-4">
                            <p class="text-sm text-gray-500 dark:text-gray-400">
                                Showing {{ pollsPagination.from }} to {{ pollsPagination.to < pollsPagination.total ? pollsPagination.to : pollsPagination.total }} of {{ pollsPagination.total }} results
                            </p>
                            <Paginator :pagination="pollsPagination"
                                @paginated="(payload: number) => handlePageChange(payload)"
                                @perPageUpdated="(payload: number) => handlePerPageChange(payload)">
                            </Paginator>
                        </div>

                        <div class="justify-center text-center rounded-lg border-2 border-dashed border-gray-300 p-6 mt-6" v-if="pollsData.length === 0">
                            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                                <path vector-effect="non-scaling-stroke" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 13h6m-3-3v6m-9 1V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2z" />
                            </svg>
                            <h3 class="mt-2 text-sm font-semibold text-gray-900 dark:text-gray-200">No polls found</h3>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">No polls match the current filter.</p>
                        </div>
                    </div>
                </div>
            </div>
...
const menuOptions = computed(() => [
    { name: 'Review', value: 'review', count: props.counts?.pendingCount ?? 0 },
    { name: 'Active', value: 'active', count: props.counts?.activeCount ?? 0 },
    { name: 'All',    value: 'all',    count: props.counts?.allPolls ?? 0 },
]);
...
const pollsData = computed(() => props.polls?.data || []);
const pollsPagination = computed(() => props.polls?.meta || props.polls || null);

const currPage = ref<number>(props.polls?.meta?.current_page || 1);
const perPage = ref<number>(props.polls?.meta?.per_page || 10);
...
const reloadData = () => {
    ...
    router.get(route('admin.polls.index'), params, {
        preserveState: false,
        preserveScroll: true,
    });
};
