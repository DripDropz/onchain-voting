...
test('test super-admin can create ballot', function() {
    ...
    $payload = ['title'=>"super-admin ballot", 'version'=>"super-admin version", 'status'=>'published', 'type'=>'snapshot'];
    $mockRequest = Request::create('ballots/create', 'POST', $payload);
    $response = (new BallotController)->store($mockRequest);
    $createdBallot = Ballot::where('version', 'super-admin version')->first();
    ...
    expect(Ballot::where('version', 'super-admin version')->first())->toBeTruthy();
...
test('test admin cannot create ballot', function() {
    ...
    $payload = ['title'=>"admin ballot", 'version'=>"admin version", 'status'=>'published', 'type'=>'snapshot'];
    $mockRequest = Request::create('ballots/create', 'POST', $payload);
    $response = (new BallotController)->store($mockRequest);
...
test('test user cannot create ballot', function() {
    ...
    $payload = ['title'=>"user ballot", 'version'=>"user version", 'status'=>'published', 'type'=>'snapshot'];
    $mockRequest = Request::create('ballots/create', 'POST', $payload);
    $response = (new BallotController)->store($mockRequest);
...
test('test ballot can be edited before started_at date', function() {
    ...
    $payload = ['title'=>"updated ballot title", 'description'=>'updated ballot description', 'version' => 'new updated version'.random_int(1,5000)];
    $mockRequest = Request::create('ballots/'.$existingBallot->hash.'/create', 'PATCH', $payload);
    $response = (new BallotController)->update($mockRequest, $ballot=$existingBallot->hash);
    $updatedBallot = Ballot::where('title', $payload['title'])->first();

    // assert ballot update occured
    assertEquals(302, $response->status());
...
test('test ballot cannot be edited after started_at date', function() {
    ...
    $payload = ['title'=>"updated ballot title", 'description'=>'updated ballot description', 'version' => 'new updated version'.random_int(1,5000)];
    $mockRequest = Request::create('ballots/'.$existingBallot->hash.'/create', 'PUT', $payload);
    $response = (new BallotController)->update($mockRequest, $existingBallot->hash);
    $updatedBallot = Ballot::byHash($existingBallot->hash);

    // assert ballot update never occured
    assertEquals(302, $response->status());
...
{
    $existingBallot = Ballot::factory()->create([
    ...
    return Ballot::where('version', $existingBallot->version)->first();
}
